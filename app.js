...
let passangers = [new Passenger('New Max', 2341), new Passenger('Fire Andrew', 1234),
    new Passenger('Full Spice', 7686)]    
...
app.get('/pass', (req, res)=>{
    res.render('pass', {data: passangers})
})
app.get('/trains', (req, res)=>{
    res.render('trains', {data: trains})
})
app.get('/buyTicket', (req, res)=>{
    res.render('buyTicket', {trains: trains, pass: passangers, ticket: tickets, buyTicket: buyTicket})
})
app.get('/reports', (req, res)=>{
    let result = {};
    let resultMoney = {};
    console.log(resultMoney);
    buyTicket.forEach(value => {
    ...
    trains.forEach(value => {
        let test = buyTicket.some(valueTwo => valueTwo.train.Route === value.Route )
...
app.post('/findPass', (req, res)=>{
    ...
    if(check) {
        res.render('pass', {info: pass, data: passangers})
...
app.post('/buyTicket', (req, res)=> {
    ...
    const dates = [new Date(2020, 2, 12, 10, 30, 30)]
    let date = dates[0];
    let newbuyTicket = new TicketsSold(passangers[pass], trains[train], tickets[ticket], date);
    let check=false;
    for(let i=0;i<buyTicket.length;i++) {
        if(buyTicket[i].ticket===newbuyTicket.ticket&&buyTicket[i].train===newbuyTicket.train){
            check=true;
        }
    }
    if (!check) {
        buyTicket.push(newbuyTicket)
    }
    res.render('buyTicket', {trains: trains, pass: passangers, ticket: tickets, buyTicket: buyTicket})
});
app.post('/changeTicket', (req, res)=>{
    let body = req.body;
    console.log(body);
    let buyTicketIndex = body.buyTicket[0];
    let ticket = body.ticket[0];
    buyTicket[buyTicketIndex].ticket = tickets[ticket];
    res.redirect('buyTicket')
});
...
app.get('/allPass', (req, res)=>{
    res.render('collections/passenger', {data: passangers})
})
app.get('/allTrains', (req, res)=>{
    res.render('collections/trains', {data: trains})
})
app.get('/allTickets', (req, res)=>{
    res.render('collections/ticket', {data: tickets})
})
